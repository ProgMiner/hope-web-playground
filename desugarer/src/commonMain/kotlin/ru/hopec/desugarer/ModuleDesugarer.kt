...
open class ModuleDesugarer(
    ...
    fun resolveModule(module: AstNode.Module): DesugaredRepresentation.Module {
        ...
        val publicData = publicDataTypes.map { name ->
        ...
        val privateData = moduleContext.moduleDataTypes.map { it.value }.toSet().minus(publicDataTypes.toSet())
    ...
    protected fun resolveDataDecl(data: AstNode.DataDeclaration, module: String?):
        ...
        return dataName to Data(
                data.dataConstructors.map { (name, type) ->
    ...
    private fun resolveDataDeclType(name: String, type: AstNode.TypeExpr?, boundVars: List<String>): Pair<String, List<Type>> {
        if (type == null)
    ...
    private fun resolveFunction(data: AstNode.FunctionDeclaration): Declarations.Function {
        ...
        val lambda = Expr.Lambda(data.equations.map { equation ->
            newScope().let {
    ...
    private fun resolveExpression(expr: AstNode.Expr): Expr {
        fun listToExpr(list: List<AstNode.Expr>, nil: Declarations.Function.Name, cons: Declarations.Function.Name) : Expr =
            if (list.isNotEmpty()) {
                Expr.Application(
        ...
        return when (expr) {
            is AstNode.LambdaExpr -> {
                Expr.Lambda(expr.branches.map { branch ->
                    newScope().let { scope ->
            ...
            is AstNode.LetExpr -> {
                newScope().let { scope ->
                    ...
                    val value = newScope().resolveExpression(expr.value)
    ...
    private fun resolveType(type: AstNode.TypeExpr, boundVars: List<String>): Type =
        when(type) {
            is AstNode.ProductType -> Type.Data.tuple(resolveType(type.left, boundVars), resolveType(type.right, boundVars))
            is AstNode.FunctionalType -> Type.Arrow(resolveType(type.premise, boundVars), resolveType(type.result, boundVars))
            ...
            is AstNode.VarType -> Type.Variable(boundVars.size - boundVars.indexOf(type.name))
        }
    ...
    fun extendModuleFunction(name: String, function: Declarations.Function.Name.User) {
        moduleContext.extendModuleFunction(name, function)
    }

    fun extendModuleConstructor(name: String, constructor: Constructor) {
    ...
    fun importModule(name: String) {
        val declarations = globalContext.moduleDeclarations[name]
    ...
    fun resolvePattern(name: String) =
        moduleContext.resolvePattern(name) ?: throw IllegalArgumentException("$name not found")

    fun resolvePatternNullary(name: String) =
        moduleContext.resolvePattern(name) ?: localContext.resolvePatternVar(name)

    fun resolveData(name: String) =
        moduleContext.resolveData(name) ?: throw IllegalArgumentException("$name not found")

    private fun String.generateNewName() =
        (this + "_" + identCounter.toString()).also { identCounter++ }
}

private fun ModuleDesugarer.newScope() =
    ModuleDesugarer(this.globalContext, this.moduleContext, this.localContext.copy())
