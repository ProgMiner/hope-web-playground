...
private class JavaTreeCursor(tree: JavaTree, val delegate: TSTreeCursor) : TsTreeCursor {
    ...
    override fun gotoFirstChildForIndex(goalIndex: UInt): Boolean =
        delegate.gotoFirstChildForByte(goalIndex.toInt()) != -1

    override fun gotoFirstChildForPosition(goalPosition: TsPoint): Boolean =
        delegate.gotoFirstChildForPoint(goalPosition.inner()) != -1
...
private class JavaPoint(val delegate: TSPoint) : TsPoint {
    override val row: UInt = delegate.row.toUInt()
    override val column: UInt = delegate.column.toUInt()
}

private class JavaRange(delegate: TSRange) : TsRange {
    override val startIndex: UInt = delegate.startByte.toUInt()
    override val endIndex: UInt = delegate.endByte.toUInt()
    override val startPosition: TsPoint = JavaPoint(delegate.startPoint)
    override val endPosition: TsPoint = JavaPoint(delegate.endPoint)
...
private class JavaQuery(val delegate: TSQuery) : TsQuery {
    ...
    override fun isPatternGuaranteedAtStep(byteOffset: UInt): Boolean =
        delegate.isPatternGuaranteedAtStep(byteOffset.toInt())
    ...
    override fun startIndexForPattern(patternIndex: UInt): UInt =
        delegate.getStartByteForPattern(patternIndex.toInt()).toUInt()
...
private class JavaLookaheadIterator(val delegate: TsLookAheadIterator) : TsLookaheadIterator {
    override val currentTypeId: UInt = delegate.currentSymbol().toUInt()

    override val currentType: String = delegate.currentSymbolName()

    override fun reset(language: TsLanguage, stateId: UInt): Boolean = delegate.reset(language.inner(), stateId.toInt())

    override fun resetState(stateId: UInt): Boolean = delegate.resetState(stateId.toInt())
}

private class JavaLanguage(val delegate: TSLanguage) : TsLanguage
