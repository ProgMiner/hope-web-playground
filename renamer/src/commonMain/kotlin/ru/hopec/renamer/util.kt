...
fun <T, V : T> parseMultipleOrError(node: TsSyntaxNode, parseFunc: (TsSyntaxNode) -> T?, orElse: (RenamerException) -> V, from: UInt = 0u, to: UInt? = null): List<T> {
    val list = mutableListOf<T>()
    for (i in from until (to ?: node.namedChildCount)) {
        try {
            parseFunc(node.getChildOrThrow(i))?.let { list.add(it) }
...
fun <T> parseMultipleOrNull(node: TsSyntaxNode, parseFunc: (TsSyntaxNode) -> T?, from: UInt = 0u, to: UInt? = null) =
