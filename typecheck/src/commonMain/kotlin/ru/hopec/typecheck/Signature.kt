...
import ru.hopec.typecheck.TypedRepresentation.Declarations.Data.Name.*
...
internal data class Signature(
    ...
    fun extendLocal(other: DesugaredRepresentation.Module): Signature {
        return extend(other.public).extend(other.private)
    }

    fun extend(other: DesugaredRepresentation.Module): Signature {
        return extend(other.public)
    }

    fun extendAll(others: Iterable<DesugaredRepresentation.Module>): Signature =
        others.fold(this) { acc, cur -> acc.extend(cur) }
    ...
    companion object {
        val core = Signature(
            mapOf(
                ...
                Declarations.Function.Name.Constructor(Core.List, "cons") to PolymorphicType(
                    Type.Arrow(
                ...
                Declarations.Function.Name.Constructor(Core.Tuple, "#") to PolymorphicType(
                    Type.Arrow(
                        Type.Variable(0),
                        Type.Arrow(Type.Variable(1), Type.Data.tuple(Type.Variable(0), Type.Variable(1))),
                    ), 2
                ),
                Declarations.Function.Name.Core("+") to PolymorphicType(
                    Type.Arrow(
                        Type.Data.tuple(
                            Type.Data.num,
                            Type.Data.num
            ...
            mapOf(
                ...
                Core.TruVal to Declarations.Data(
                    mapOf(
                        "true" to listOf(),
                        "false" to listOf(),
                    ), 0
                ),
                Core.List to Declarations.Data(
                    mapOf(
                        "nil" to listOf(),
                        "cons" to listOf(Type.Variable(0)),
                    ), 1
                ),
                Core.Set to Declarations.Data(
                    mapOf(
                        "emptySet" to listOf(),
                    ), 1
                ),
                Core.Tuple to Declarations.Data(
                    mapOf(
