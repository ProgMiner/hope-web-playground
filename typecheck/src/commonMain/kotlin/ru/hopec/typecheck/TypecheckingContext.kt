...
internal class TypecheckingContext private constructor(val signature: Signature) {
    private data class BoundVariable(val typeVariable: Int, val letRange: LetRange)

    companion object {
        fun runModule(signature: Signature, module: DesugaredRepresentation.Module): TypedRepresentation.Module =
            TypecheckingContext(signature).runModule(module)

        fun runDeclarations(
            signature: Signature,
            declarations: DesugaredRepresentation.Declarations
        ): TypedRepresentation.Declarations =
            TypecheckingContext(signature).runDeclarations(declarations)

        fun runFunction(
            signature: Signature,
            function: DesugaredRepresentation.Declarations.Function
        ): TypedRepresentation.Declarations.Function? =
            TypecheckingContext(signature).runFunction(function)
    }
    ...
    private fun runDeclarations(declarations: DesugaredRepresentation.Declarations): TypedRepresentation.Declarations =
        TypedRepresentation.Declarations(
            declarations.data,
            declarations.functions.toList().flatMap { (k, v) -> runFunction(v).nullAsList().map { k to it } }.toMap()
        )
    ...
    private fun runFunction(function: DesugaredRepresentation.Declarations.Function): TypedRepresentation.Declarations.Function? {
        ...
        var unifyOk = true
        fun unify(actual: Type, target: Type) {
            when (actual) {
                is Type.Variable -> {
                    val image = variableMapping[actual.index]
                    if (image == null) variableMapping[actual.index] = target
                    else unifyOk = unifyOk && image == target
                }
        ...
        fun rename(type: Type): Type =
            when (val walked = walk(type)) {
                is Type.Variable -> if (walked.index < 0) {
                    walked
                } else {
        ...
        fun rename(pattern: TypedRepresentation.Pattern): TypedRepresentation.Pattern =
            when (pattern) {
                is TypedRepresentation.Pattern.Data -> TypedRepresentation.Pattern.Data(
                    rename(pattern.type) as Type.Data, pattern.constructor, pattern.args.map(::rename).toList()
                )

                is TypedRepresentation.Pattern.Variable -> TypedRepresentation.Pattern.Variable(
                    rename(pattern.type), pattern.name
                )

                is TypedRepresentation.Pattern.Wildcard -> TypedRepresentation.Pattern.Wildcard(rename(pattern.type))
                is TypedRepresentation.Pattern.NamedData -> TypedRepresentation.Pattern.NamedData(
                    pattern.name, rename(pattern.data) as TypedRepresentation.Pattern.Data
                )
            }
        ...
        fun rename(term: TypedRepresentation.Expr): TypedRepresentation.Expr =
            when (term) {
                ...
                is TypedRepresentation.Expr.If -> TypedRepresentation.Expr.If(
                    rename(term.condition), rename(term.positive), rename(term.negative)
                )

                is TypedRepresentation.Expr.Variable -> TypedRepresentation.Expr.Variable(
                    rename(term.type), term.name
                )

                is TypedRepresentation.Expr.Lambda -> TypedRepresentation.Expr.Lambda(
                    rename(term.type) as Type.Arrow, term.branches.map {
                        TypedRepresentation.Expr.Lambda.Branch(rename(it.pattern), rename(it.body))
                    }
                )

                is TypedRepresentation.Expr.Let -> TypedRepresentation.Expr.Let(
                    rename(term.pattern), rename(term.matcher), rename(term.body)
                )

                is TypedRepresentation.Expr.Identifier -> TypedRepresentation.Expr.Identifier(
                    rename(term.type), term.name
                )

                is TypedRepresentation.Expr.Application -> TypedRepresentation.Expr.Application(
                    rename(term.type), rename(term.left), rename(term.right)
                )
            }
    ...
    private fun infer(term: DesugaredRepresentation.Expr): TypedRepresentation.Expr? {
        return when (term) {
            ...
            is DesugaredRepresentation.Expr.Lambda -> {
                ...
                val branches = term.branches.map { branch ->
                    val (pattern, body) = withBoundPattern(branch.pattern, nullRage) {
                        infer(branch.body)
                    } ?: return null
            ...
            is DesugaredRepresentation.Expr.Let -> {
                ...
                val (pattern, body) = withBoundPattern(term.pattern, range) { pattern ->
            ...
            is DesugaredRepresentation.Expr.If -> {
                ...
                (unify(condition?.type, Type.Data.truval) join unify(
                    positive?.type,
                    negative?.type,
                )) ?: return null
    ...
    private fun bindPattern(
        ...
        return when (pattern) {
            is DesugaredRepresentation.Pattern.Data -> {
                ...
                val patternArgs = pattern.args.zip(args.map { it.shift(shift) }).map { (pat, arg) ->
        ...
        return when (walkedLeft) {
            ...
            is Type.Arrow -> {
                when (walkedRight) {
                    ...
                    is Type.Arrow -> unify(walkedLeft.argument, walkedRight.argument) join unify(
                        walkedLeft.result,
                        walkedRight.result
                    )
            ...
            is Type.Data -> {
                when (walkedRight) {
                    ...
                    is Type.Data -> {
                        ...
                        walkedLeft.args.zip(walkedRight.args).map { (left, right) ->
                            unify(left, right)
                        }.joinAll()
                        Unit
                    }
    ...
    fun rangeContains(range: Pair<Int, Int>, binder: Int) = binder in range.first..<range.second

    private fun refresh(binder: Int): Type {
        val range = boundVariables[binder].letRange
        val varMapping = hashMapOf<Int, Int>()

        fun go(type: Type): Type = when (val wtype = walk(type)) {
    ...
    private fun walk(type: Type): Type = when (type) {
        is Type.Variable -> {
