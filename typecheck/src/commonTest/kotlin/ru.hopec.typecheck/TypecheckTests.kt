...
private fun polymorphic(type: Type): TypedRepresentation.PolymorphicType {
    fun maxBinder(type: Type): Int = when (type) {
...
private fun typedVar(type: Type, name: String) = TypedRepresentation.Expr.Variable(type, name)
private fun dsVar(name: String, binder: Int) = DesugaredRepresentation.Expr.Variable(name, binder)
private fun typedPatVar(type: Type, name: String) = TypedRepresentation.Pattern.Variable(type, name)
private fun dsPatVar(name: String) = DesugaredRepresentation.Pattern.Variable(name)
private fun branches(vararg branches: Pair<TypedRepresentation.Pattern, TypedRepresentation.Expr>) = branches.map {
    TypedRepresentation.Expr.Lambda.Branch(it.first, it.second)
}

private val constructorTrue = Declarations.Function.Name.Constructor(
    Declarations.Data.Name.Core.TruVal,
    "true"
)

private val constructorFalse = Declarations.Function.Name.Constructor(
    Declarations.Data.Name.Core.TruVal,
    "false"
)
...
private fun dsTuple(left: DesugaredRepresentation.Expr, right: DesugaredRepresentation.Expr) =
    (DesugaredRepresentation.Expr.Identifier(makeTuple) dsAp left) dsAp right

private fun dsTuplePat(left: DesugaredRepresentation.Pattern, right: DesugaredRepresentation.Pattern) =
    DesugaredRepresentation.Pattern.Data(makeTuple, listOf(left, right))

private fun tuplePat(left: TypedRepresentation.Pattern, right: TypedRepresentation.Pattern) =
    TypedRepresentation.Pattern.Data(Type.Data.tuple(left.type, right.type), makeTuple, listOf(left, right))

class TypecheckTests {
    private fun annotateGlobal(func: DesugaredRepresentation.Declarations.Function) =
        TypecheckingContext.runFunction(Signature.core, func)

    @Test
    fun smoke() {
        val desugId = DesugaredRepresentation.Declarations.Function(
            DesugaredRepresentation.Expr.Lambda(
                dsBranches(dsPatVar("x") to dsVar("x", 0))
            ),
            polymorphic(typeVar(0) arrow typeVar(0)),
        )
        val typedId = TypedRepresentation.Declarations.Function(
            TypedRepresentation.Expr.Lambda(
                typeVar(0) arrow typeVar(0), branches(
    ...
    @Test
    fun letPolymorphism() {
        val poly = DesugaredRepresentation.Declarations.Function(
            DesugaredRepresentation.Expr.Lambda(
                dsBranches(
                    dsPatVar("x") to DesugaredRepresentation.Expr.Let(
                        dsPatVar("idf"),
                        DesugaredRepresentation.Expr.Lambda(
                            dsBranches(dsPatVar("x") to dsVar("x", 0))
                        ),
        ...
        assertEquals(
            ...
            (result.lambda.branches.first().body as TypedRepresentation.Expr.Let).matcher.type
        )
    }

    @Test
    fun complexPattern() {
        val desugFunction = DesugaredRepresentation.Declarations.Function(
            DesugaredRepresentation.Expr.Lambda(
                dsBranches(
    ...
    fun multipleBranches() {
        val desugIsNil = DesugaredRepresentation.Declarations.Function(
            DesugaredRepresentation.Expr.Lambda(
                dsBranches(
        ...
        val typedIsNil = TypedRepresentation.Declarations.Function(
            TypedRepresentation.Expr.Lambda(
